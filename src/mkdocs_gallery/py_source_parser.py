...
#
#     b = 2
INFILE_CONFIG_PATTERN = re.compile(r"^[\ \t]*#\s*mkdocs_gallery_([A-Za-z0-9_]+)(\s*=\s*(.+))?[\ \t]*\n?", re.MULTILINE)
...
def _get_docstring_and_rest(file: Path):
    ...
    if not isinstance(node, ast.Module):
        raise ExtensionError("This function only supports modules. " "You provided {0}".format(node.__class__.__name__))
...
def extract_file_config(content):
    ...
    for match in re.finditer(INFILE_CONFIG_PATTERN, content):
        ...
        try:
            ...
            logger.warning("mkdocs-gallery option %s was passed invalid value %s", name, value)
        else:
...
def split_code_and_text_blocks(
    source_file: Union[str, Path], return_node=False
