...
def extra_readme_title(filename, contents):
    """Same as `extract_intro_and_title` for the readme files in galleries."""

    match = re.search(r'^[# ]*(?!([\W _])\1{3,})[# ]*(.+)', contents,
                      re.MULTILINE)
    if match is None:
        raise ExtensionError(f"Could not find a title in readme file: {filename}")
...
def save_thumbnail(image_path_template, src_file, script_vars, file_conf,
    ...
    if thumbnail_number is None:
        ...
        if not isinstance(thumbnail_number, int):
            raise ExtensionError(
                'mkdocs_gallery_thumbnail_number setting is not a number, '
...
def generate_dir(gallery_src_dir, subgallery_src_dir, target_dir, gallery_conf, seen_backrefs):
    """
    ...
    ----------
    gallery_src_dir : str
        The source folder of the current gallery. For example <project>/docs/examples/

    subgallery_src_dir : str
        The source folder of the current subsection to handle, containing the python files and the readme.md.
        For example <project>/docs/examples/subgallery.

    target_dir : str
        The target folder where the documents, notebooks and images from this sub-gallery should be generated.
        For example <project>/docs/generated/gallery/subgallery
    ...
    md_files : Dict[str, str]
        A map of title > paths to the generated markdown files, in order of inclusion in the toc.

    fhindex : str
        The markdown to include in the global readme.

    costs : List[Tuple[Tuple[float, float], str]
        A list of ((execution time, memory cost), python file path) for all python files in subgallery_src_dir.
    """
    md_files = dict()
    subsection_path = os.path.relpath(subgallery_src_dir, gallery_src_dir)

    head_ref = os.path.relpath(target_dir, gallery_conf['src_dir'])
    fhindex = f"<!--{head_ref.replace(os.path.sep, '_')}-->\n"

    # Read the gallery readme
    fname = _get_readme(subgallery_src_dir, gallery_conf)
    with codecs.open(fname, 'r', encoding='utf-8') as fid:
         readme_contents = fid.read()
    readme_title = extra_readme_title(fname, readme_contents)

    # Add empty lines to avoid bug in issue #165
    fhindex += readme_contents
    fhindex += "\n\n"

    if not os.path.exists(target_dir):
        os.makedirs(target_dir)
    # get filenames
    listdir = [fname for fname in os.listdir(subgallery_src_dir)
    ...
    listdir = [fname for fname in listdir
               if re.search(gallery_conf['ignore_pattern'],
                            os.path.normpath(os.path.join(subgallery_src_dir, fname)))
               is None]
    # sort them
    sorted_listdir = sorted(
        listdir, key=gallery_conf['within_subsection_order'](subgallery_src_dir))
    entries_text = []
    costs = []
    build_target_dir = os.path.relpath(target_dir, gallery_conf['src_dir'])
    # iterator = mkdocs_compatibility.status_iterator(...
    for fname in tqdm(sorted_listdir, desc=f"generating gallery for {build_target_dir}... "):
        intro, title, cost = generate_file_md(
            fname, target_dir, subgallery_src_dir, subsection_path, gallery_conf, seen_backrefs)
        src_file = os.path.normpath(os.path.join(subgallery_src_dir, fname))
        costs.append((cost, src_file))
        # Create the thumbnail to appear in the readme
        this_entry = _thumbnail_div(target_dir, gallery_conf['src_dir'], subsection_path,
                                    fname, intro, title)
        entries_text.append(this_entry)

        # Create the toc entry
        md_files[title] = os.path.join(build_target_dir, fname[:-3] + ".md").replace(os.sep, '/')

    for entry_text in entries_text:
        fhindex += entry_text

    # clear at the end of the section
    fhindex += """<div class="mkd-glr-clear"></div>\n\n"""

    return readme_title, md_files, fhindex, costs
...
def handle_exception(exc_info, src_file, script_vars, gallery_conf):
    ...
    script_vars['parse_and_execute_if_needed'] = False
...
def _check_reset_logging_tee(src_file):
    # Helper to deal with our tests not necessarily calling parse_and_execute_if_needed
...
def _get_code_output(is_last_expr, example_globals, gallery_conf, logging_tee,
    ...
    code_output = u"\n{0}\n\n{1}\n{2}\n\n".format(
        images_md, captured_std, captured_html
    )
    return code_output
...
def execute_code_block(compiler, block, example_globals, script_vars,
    ...
    if not script_vars['parse_and_execute_if_needed'] or blabel == 'text':
        return ''
    ...
    try:
        ...
        if need_save_figures:
            ...
            images_md = save_figures(block, script_vars, gallery_conf)
        else:
            images_md = u''
    except Exception:
        logging_tee.restore_std()
        except_md = handle_exception(
...
def executable_script(src_file, gallery_conf):
    """Validate if script has to be run according to gallery configuration: filename_pattern and global plot_gallery
...
def parse_and_execute_if_needed(script_blocks, script_vars, gallery_conf):
    ...
    argv_orig = sys.argv[:]
    if script_vars['parse_and_execute_if_needed']:
        # We want to run the example without arguments. See
        # https://github.com/mkdocs-gallery/mkdocs-gallery/pull/252
        # for more details.
...
def generate_file_md(fname, target_dir, src_dir, subsection_path, gallery_conf,
                     seen_backrefs=None):
    ...
    # Extract the contents of the title+intro
    ...
    # Does the script have to be run ? According to configuration
    executable = executable_script(src_file, gallery_conf)

    # If there were no changes, possibly skip
    ...
    # Init the images file path template
    base_image_name = os.path.splitext(fname)[0]
    image_fname = 'sphx_glr_' + base_image_name + '_{0:03}.png'
    image_path_template = os.path.join(image_dir, image_fname)

    script_vars = {
        'parse_and_execute_if_needed': executable,
    ...
    # Execute the script per se, and format the output blocks as Mardown
    output_blocks, time_elapsed = parse_and_execute_if_needed(script_blocks, script_vars, gallery_conf)
    if executable:
        logger.debug("%s ran in : %.2g seconds\n", src_file, time_elapsed)
    else:
        logger.debug("%s parsed (not executed)\n", src_file)

        # Create dummy images if not executable
        dummy_image = file_conf.get('dummy_images', None)
        if dummy_image is not None:
            if type(dummy_image) is not int:
                raise ExtensionError(
                    'mkdocs_gallery_dummy_images setting is not a number, '
                    'got %r' % (dummy_image,))
    ...
    example_md = md_blocks(script_blocks, output_blocks, file_conf, gallery_conf)
    memory_used = gallery_conf['memory_base'] + script_vars['memory_delta']
    if not executable:
        time_elapsed = memory_used = 0.  # don't let the output change
    save_md_example(example_md, target_file, time_elapsed, memory_used,
                    gallery_conf)
    ...
    _write_backreferences(backrefs, seen_backrefs, gallery_conf, target_dir,
                          subsection_path, fname, intro, title)
...
def md_blocks(script_blocks, output_blocks, file_conf, gallery_conf):
    ...
    for bi, ((blabel, bcontent, lineno), code_output) in \
        ...
        # do not add comment to the title block (bi=0), otherwise the linking
        ...
        if blabel == 'code':
            ...
            code_md = codestr2md(bcontent, lang=gallery_conf['lang'],
                                 lineno=lineno) + '\n'
...
def save_md_example(example_md, example_file, time_elapsed,
                    memory_used, gallery_conf):
    """Saves the md notebook to example_file including header & footer

    Parameters
    ----------
    example_md : str
        md containing the executed file content
    example_file : str
    ...
    example_md = EXAMPLE_HEADER.format(
        pyfile=example_fname, opt_binder_text=binder_text) + example_md

    if time_elapsed >= gallery_conf["min_reported_time"]:
        time_m, time_s = divmod(time_elapsed, 60)
        example_md += TIMING_CONTENT.format(time_m, time_s)
    if gallery_conf['show_memory']:
        example_md += ("**Estimated memory usage:** {0: .0f} MB\n\n"
                        .format(memory_used))

    # Generate a binder URL if specified
    binder_badge_md = ''
    if len(binder_conf) > 0:
        binder_badge_md += gen_binder_md(example_file, binder_conf,
                                           gallery_conf)

    fname = os.path.basename(example_file)

    # Embedding the icon is too difficult for now, rely on mkdocs-material
    # icons_relative_path = os.path.relpath(gallery_conf['src_dir'], os.path.dirname(example_file))
    # iconpath = os.path.join(icons_relative_path, 'download_solid.svg').replace(os.path.sep, '/')
    # icon = f"![fontawesome-solid-download]({iconpath})"
    icon = ":fontawesome-solid-download:"

    example_md += CODE_DOWNLOAD.format(py_name=fname, download_icon=icon,
                                       ipynb_name=replace_py_ipynb(fname),
                                       binder_badge_md=binder_badge_md,
                                       ref_fname=ref_fname)
    example_md += SPHX_GLR_SIG

    write_file_new = re.sub(r'\.py$', '.md.new', example_file)
